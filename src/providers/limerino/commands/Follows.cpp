             ...
             onProgress](const QJsonObject &data) {
                const QJsonObject conn =
                    data[QStringLiteral("user")]
                        .toObject()[followersMode ? QStringLiteral("followers")
                                                  : QStringLiteral("follows")]
                        .toObject();
                const QJsonArray edges =
                    conn[QStringLiteral("edges")].toArray();
                if (edges.isEmpty())
                {
                    onRows(*rows);
                    return;
                ...
                ++(*pages);
                const bool repeated =
                    !nextCursor.isEmpty() && seenCursors->contains(nextCursor);
        ...
        [weak = std::weak_ptr(channel)](const QJsonObject & /*data*/) {
            if (auto chan = weak.lock())
            {
                ...
                chan->addSystemMessage(
                    QStringLiteral("Follow request successful!"));
            }
        },
...
QString follow(const CommandContext &ctx)
{
    ...
    gql::executePersisted(
        ...
        token.token,
        [weak = std::weak_ptr(channel), target,
         token = token.token](const QJsonObject &data) {
...
void openFollowerListFor(Split *split)
{
    ...
    dialog->resultList()->setColumns({QStringLiteral("user"),
                                      QStringLiteral("following since"),
                                      QStringLiteral("for")});
    ...
    dialog->resultList()->setRowMenuProvider(
        [g = QPointer<limerino::LimerinoResultDialog>(dialog),
         token = token.token](const QStringList &row, QMenu *menu) {
            ...
            menu->addAction(
                QStringLiteral("Block %1").arg(user),
                [g, user, token]() mutable {
                    gql::executePersisted(
                            ...
                            if (id.isEmpty())
                            {
                                if (g)
                                {
                                    g->resultList()->setStatusText(
                                        QStringLiteral("could not resolve %1")
                                            .arg(user));
                                }
                                return;
                            }
                            const auto app = getApp();
                            app->getAccounts()->twitch.getCurrent()->blockUser(
                                id, user, g,
                                [g, user] {
                                    if (g)
                                    {
                                ...
                                [g, user] {
                                    ...
                                    {
                                        g->resultList()->setStatusText(
                                            QStringLiteral("could not block %1")
                                                .arg(user));
                                    }
                                });
                        },
                        [g, user](const gql::GqlError &e) {
                            if (g)
                            {
                                g->resultList()->setStatusText(e.message);
                            }
                        });
                });
        });
    ...
    fetchFollows(
        ...
        [g = QPointer<limerino::LimerinoResultDialog>(dialog)](
            ...
            for (const FollowRow &r : rows)
            {
                ...
                out.append({r.login,
                            dt.isValid() ? dt.toString(QStringLiteral(
                                               "yyyy-MM-dd hh:mm"))
            ...
            g->resultList()->setStatusText(
                QStringLiteral("%1 followers").arg(rows.size()));
        },
        [g = QPointer<limerino::LimerinoResultDialog>(dialog)](
            const QString &err) {
            if (g)
...
void openFollowingListFor(Split *split)
{
    ...
    dialog->resultList()->setRowMenuProvider([](const QStringList &row,
                                                QMenu *menu) {
        const QString user = row.value(0);
        ...
        menu->addAction(QStringLiteral("Open channel in browser"), [user] {
            QDesktopServices::openUrl(
                QUrl(QStringLiteral("https://www.twitch.tv/%1").arg(user)));
        });
    });
