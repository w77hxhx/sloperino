        ...
        if (fakeMessage != nullptr && fakeMessage->command() == "PRIVMSG")
        {
            ...
            if (builtMessage)
            {
                ...
                builtMessage->elements.erase(
                    std::remove_if(
                        builtMessage->elements.begin(),
                        builtMessage->elements.end(),
                        [](const std::unique_ptr<MessageElement> &el) {
                            return dynamic_cast<TimestampElement *>(el.get()) !=
                                   nullptr;
                        }),
                    builtMessage->elements.end());
    ...
    , closeAutomatically_(closeAutomatically)
{
    ...
    auto user = layout.emplace<QHBoxLayout>().withoutMargin();
    {
        ...
        QObject::connect(nameHistory.getElement(), &Button::leftClicked,
                         [this] {
                             // No parent: the result dialog must be an
                             // independent top-level (see upstream comment).
        ...
        // Limerino fork hooks: artist / unartist / lead mod (own channel
        // only). Same PixmapButton construction as mod/unmod (30x30).
...
void UserInfoPopup::showLogDateMenu()
{
    ...
    for (size_t i = 0; i < this->zonianLogMonths_.size(); ++i)
    {
        ...
        const bool isSelected = (!this->isZonianSearchMode_ &&
                                 i == this->zonianSelectedMonthIndex_);
        const QString labelText = isSelected
        ...
        QObject::connect(
            itemBtn, &QPushButton::clicked, this, [this, popup, i] {
                popup->close();
                if (this->ui_.logSearchInput)
                {
                    QSignalBlocker blocker(this->ui_.logSearchInput);
...
void UserInfoPopup::fetchZonianLogList()
{
    ...
    QString channelName;
    if (this->underlyingChannel_ && this->underlyingChannel_->isTwitchChannel())
    {
        channelName = this->underlyingChannel_->getName();
    }
    else if (this->channel_ && this->channel_->isTwitchChannel())
    {
    ...
    NetworkRequest(url)
        ...
        .onSuccess([self, generation, chan, user](const NetworkResult &result) {
            ...
            {
                self->zonianSelectedMonthIndex_ = 0;
        ...
        .onError([self, generation](const NetworkResult &result) {
            ...
            if (self->ui_.logDateButton)
            {
                ...
                self->ui_.logDateButton->setToolTip(
                    isRateLimit
                        ? QStringLiteral(
                              "Log rate limit reached. Please wait a moment.")
                        : QStringLiteral("No logs available"));
                self->ui_.logDateButton->show();
                self->updateLogControlsStyle();
            }
...
void UserInfoPopup::fetchZonianMonthLog(size_t monthIndex)
{
    ...
    this->zonianSelectedMonthIndex_ = monthIndex;
    const auto monthData = this->zonianLogMonths_[monthIndex];
    if (this->ui_.logDateButton)
    {
        this->ui_.logDateButton->setText(QStringLiteral("%1-%2 ▾").arg(
...
void UserInfoPopup::searchUserLogs(const QString &query)
{
    ...
    if (searchCacheIt != this->zonianSearchCache_.end())
    {
        ...
        if (this->ui_.logDateButton)
        {
            ...
            this->ui_.logDateButton->setToolTip(
                QStringLiteral("Searching all logs (click or scroll to select "
                               "a specific month)"));
            this->ui_.logDateButton->show();
            this->updateLogControlsStyle();
        }
    ...
    NetworkRequest(url)
        ...
        .onError([self, generation](const NetworkResult &result) {
            ...
            {
                self->zonianErrorMessage_ =
                    QStringLiteral("Rate limit exceeded for log search. Please "
                                   "wait a moment.");
                if (self->ui_.logDateButton)
                {
                    self->ui_.logDateButton->setText(
                ...
                if (self->usercardMessagesChannel_)
                {
                    self->usercardMessagesChannel_->addSystemMessage(
                        QStringLiteral(
                            "Rate limit reached for search requests. "
                            "Please wait a moment before trying "
                            "again."));
                }
            }
...
void UserInfoPopup::updateUserData()
{
    ...
    const auto onUserFetched = [this, isCurrentRequest,
        ...
        // Limerino: feed the GQL-extras widget with a fresh target.
        ...
        {
            // Limerino: fresh target - drop the previous card's sub-age base
            // so a late extrasChanged() can't append onto stale text.
                ...
                getIvr()->getSubage(
                    ...
                    [this, isCurrentRequest](const IvrSubage &subageInfo) {
                        ...
                        else if (subageInfo.isSubbed)
                        {
                            ...
                            // Limerino fork hook: GQL sub detail rides on
                            // this row. Store the IVR base text separately so
...
QString UserInfoPopup::showProfilePictureContextMenu()
{
    ...
    menu->addAction("View editor-in-channels (7TV)", this, [this] {
        LimerinoCommands::showSeventvUserEditorIn(this->userName_);
    });
